...
def monitoring_kline():
    ...
    for code in codes:
        ...
        previous_day_df = df[df["datetime"].apply(lambda x: x.date() == previous_day)]

        last_close = previous_day_df.iloc[-1].close
        today_df["changes"] = 100 * (today_df["close"] - last_close) / last_close
    ...
    for time, sub_df in total_df.groupby("datetime"):
        ...
        datas.append({"code": "candidate", "time": time, "changes": mean_changes})
